...
# Note on girth sampling on the survey: they aimed for the nearest 5 mm increment as with struggling fish that
# was the best we could do with reasonable accuracy. 
...
len_gir <- read_csv("data/length_girth_2001bc.csv")
...
fshbio <- 
  read_csv(paste0("data/fishery/fishery_bio_2000_2018.csv"), guess_max = 50000) %>% 
  select(year, julian_day, trip_no, length, weight) %>% 
  filter(!is.na(length) & !is.na(weight) & year >= 2016 &
         # The application for port sampling data entry wont be ready until Sep
         # 3 (julian day = 246). Only simulate sampling base on data collected after that date.
           julian_day > 246) %>% 
...
fshbio %>% group_by(year) %>% summarize(n())
...
fshbio %>% 
  ...
  summarize(n_trips = length(unique(trip_no))) %>% 
  left_join(lencomps, by = "year") %>% 
  # number of fish in a bin that were sampled per set
  mutate(n_per_trip = n / n_trips) -> lencomps

target <- 6 # of of fish to sample in each bin per set

lencomps %>% 
  group_by(length_bin) %>% 
  mutate(mu_ntrip = mean(n_per_trip)) %>% 
  ungroup() %>% 
  mutate(no_sampled = target * n_trips,
         rate = target / n_per_trip,
         mu_rate = target / mu_ntrip,
         # adjust no_sampled when there's not enough fish in a bin to reach
         # target
         no_sampled = ifelse(rate >= 1, n, no_sampled)
         ) %>% 
  group_by(year) %>% 
...
lencomps %>% 
  group_by(year) %>% 
  distinct(n_trips, N, tot_sampled)

ggplot(data = lencomps, aes(x = length_bin, y = n_per_trip, fill = factor(year))) +
  scale_fill_grey(start = 0.3, end = 0.8) +
  geom_bar(stat = "identity", position = position_dodge(preserve = "total")) +
  geom_errorbar(aes(ymax = mu_ntrip, ymin = mu_ntrip, colour = "Mean"), lwd = 1, lty = 2) +
  labs(x = "\nLength bins", y = "Number sampled per trip\n", fill = NULL, colour = NULL)
...
ggplot(data = lencomps, aes(x = factor(year), y = rate, fill = factor(year))) +
  ...
  labs(x = "\nLength bins", y = "Sampling rate per trip\n", fill = NULL, colour = NULL) +
...
# Pot biological data
potbio <- read_csv("data/potsrv_bio_1981_2018.csv", guess_max = 50000) %>% 
